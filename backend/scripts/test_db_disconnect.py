...
def setup_test_data(db: Session):
    company = Company(name="Disconnect Test Co", code="DBDISC")
    db.add(company)
    db.flush()
    
    user = User(username="test_disc", password_hash="hash", role="ADMIN")
    db.add(user)
    db.flush()
    ...
    hub = StateHub(hub_code="DBDISC-HUB", hub_name="Test Hub", company_id=company.id)
    db.add(hub)
    db.flush()
    ...
    prod = Product(name="Disconnect Item", sku="DB-001", company_id=company.id)
    db.add(prod)
    db.flush()
...
def test_db_disconnect():
    db = SessionLocal()
    try:
        print("Setting up disconnect test data...")
        company, user, wh1, wh2, prod, inv = setup_test_data(db)
        
        req = FCDispatchBatchRequest(
            source_warehouse_id=wh1.id,
            warehouse_ids=[wh2.id],
            dispatch_type="STANDARD",
            items=[FCDispatchRequestItem(product_id=prod.id, quantity=10)]
        )
        
        # We will mock `db.add` inside InventoryEventEngine to throw an OperationalError.
        # This simulates the DB connection dropping halfway through the transaction processing.
        # It occurs AFTER FCDispatch parent and items are added to the session.
        
        original_add = db.add
        def mock_add(instance, *args, **kwargs):
            if type(instance).__name__ == "InventoryMovement":
                raise OperationalError("INSERT INTO inventory_movements", None, "Lost connection to MySQL server during query")
            return original_add(instance, *args, **kwargs)

        print("Simulating Dispatch request with mid-transaction DB disconnection...")
        try:
            with patch.object(db, 'add', side_effect=mock_add):
                FCDispatchService.create_batch_dispatch(db, company.id, req, user.id)
                db.commit()
                print("❌ FAILURE: Dispatch succeeded unexpectedly!")
        except Exception as e:
            db.rollback()
            print(f"✅ SUCCESS: Caught expected disconnection error: {e}")
        ...
        # 2. Inventory should be completely unchanged (50)
        final_inv = db.query(Inventory).filter(Inventory.id == inv.id).first()
        assert final_inv.available_qty == 50, f"Expected 50 qty, found {final_inv.available_qty}"
        
        print("✅ ROLLBACK VERIFIED: Parent records, child records, and inventory remain clean after disconnect.")
